...
export default function ClientSpace({ dark = false }: { dark?: boolean }) {
  return (
    <section id="espace-client" className={`py-12 sm:py-16 md:py-24 relative overflow-hidden ${dark ? "bg-navy text-white" : "bg-paper"}`}>
      {dark && <div className="grain" />}
      <div className="max-w-7xl mx-auto px-4 sm:px-5 md:px-6 relative">
        <div className="grid lg:grid-cols-[.82fr_1.18fr] gap-9 sm:gap-12 lg:gap-20 items-center">
          ...
          <Reveal delay={100}>
            <div className={`rounded-2xl sm:rounded-[1.75rem] p-2.5 sm:p-5 shadow-2xl shadow-black/25 ${dark ? "bg-[#090b10] border border-white/10" : "bg-navy"}`}>
              <div className="rounded-2xl bg-paperAlt border border-white/10 overflow-hidden">
                ...
                <div className="p-4 sm:p-7">
                  <div className="flex items-start justify-between gap-3 mb-5 sm:mb-6"><div className="min-w-0"><p className="text-xs text-slate">Bonjour,</p><h3 className="font-display font-bold text-lg sm:text-xl leading-tight mt-1">Votre activité en un coup d’œil</h3></div><span className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-amber text-[#111827] flex items-center justify-center flex-shrink-0"><i className="fa-solid fa-user text-sm" /></span></div>
                  <div className="grid grid-cols-3 gap-1.5 sm:gap-2.5 mb-4 sm:mb-5"><DashboardMetric value="03" label="En cours" /><DashboardMetric value="01" label="À payer" highlight /><DashboardMetric value="08" label="Terminées" /></div>
                  <div className="rounded-xl border border-ink/8 p-3 sm:p-4">
...
function DashboardMetric({ value, label, highlight = false }: { value: string; label: string; highlight?: boolean }) {
  return <div className={`min-w-0 rounded-lg sm:rounded-xl p-2 sm:p-4 ${highlight ? "bg-amber text-[#111827]" : "bg-paper border border-ink/8"}`}><p className="font-display font-bold text-base sm:text-xl">{value}</p><p className={`text-[8px] sm:text-[9px] leading-tight mt-1 break-words ${highlight ? "text-[#111827]/60" : "text-slate"}`}>{label}</p></div>;
}
